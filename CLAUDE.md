...
## Server (`server/`)
- The Node server lives in `server/` — ES modules, same "one feature one file" rule as the client. Run locally with `node server/index.js` (defaults: `127.0.0.1:8090`). `GET /health` returns 200 "ok" — keep that endpoint cheap.
- **One dependency:** [`ws`](https://github.com/websockets/ws) for the WebSocket transport. Justified because hand-rolling RFC 6455 framing/masking is ~150 lines of bug-prone boilerplate for zero gameplay benefit. Don't add more npm deps casually — every addition needs a similar paragraph here.
- Deploy pushes `server/package.json` + `server/package-lock.json` and runs `npm ci --omit=dev` on the VPS. Local: `cd server && npm install` after pulling a commit that changed deps.
